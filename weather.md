  ...
  <div id="app" class="w-full max-w-5xl pb-40 pt-24"> <!-- pt-24 to make space for alert -->
    ...
    <div class="card mb-6 p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-6 relative">
       <div class="flex items-center gap-4">
        ...
        <div class="text-center md:text-left">
          <h1 class="text-2xl md:text-3xl font-extrabold text-gray-800">
            Pagbilao Command Center <span class="text-blue-600">Weather Dashboard</span>
          </h1>
          <div class="small-muted font-medium text-gray-600">
            Municipality of Pagbilao, Quezon — 13.9714°N, 121.6869°E
          </div>
        </div>
      ...
      <div class="text-center md:text-right small-muted">
        ...
        <div id="last-updated" class="text-sm mt-1 italic">Updated: —</div>
        <div class="mt-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
      <section id="current-panel" class="card p-4">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="small-muted">Local time: <span id="local-time">—</span></div>
        </div>
        <div id="current-data" class="grid grid-cols-2 md:grid-cols-4 gap-3"></div>
        <div class="mt-4 small-muted"><strong>UV Index (today):</strong> <span id="uv-index">—</span></div>
        <div class="mt-4 small-muted"><strong>UV Max:</strong> <span id="uv-max">—</span></div>
      </section>
      <section class="card p-4">
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-bold text-gray-800 flex items-center gap-2"><i data-lucide="clock" class="w-5 h-5 icon-yellow"></i> Hourly (next 24h) 🌡️Temp | 🌧️Rain Chance |☁️Cloud Cover|🌬️Wind|💧Humidity</h2>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6" id="map-satellite-section">
      <div class="card p-4">
        ...
        <div id="provider-area" class="border rounded overflow-hidden" style="min-height:360px;">
          <div id="provider-loading" class="p-4 small-muted" hidden>Loading provider…</div>
      ...
      <div class="card p-4">
        ...
        <div id="satellite" class="text-center">
          ...
          <div class="mt-3 space-y-1">
            <a href="https://www.pagasa.dost.gov.ph/weather/weather-advisory" target="_blank" class="text-blue-600 underline"> 📢PAGASA Weather Advisory</a><br>
            <a href="https://www.pagasa.dost.gov.ph/tropical-cyclone/severe-weather-bulletin/1" target="_blank" class="text-red-600 underline">📢PAGASA Severe Weather Bulletin</a>
          </div>
...
<footer class="disclaimer-footer fixed bottom-10 left-0 right-0 z-40 bg-white/95 backdrop-blur border-t border-gray-200 shadow-md text-center text-sm text-gray-700 py-4">
  ...
  <!-- Hotline marquee -->
  <div class="marquee">
    <span>📞 HOTLINES — MDRRMO 09186244564 | 🚒 BUMBERO 09234424945 | 👮 PULIS 09985985764 | 🏛 LGU PAGBILAO (042) 797-0937 | For verified and official updates, please refer to: MDRRMO Pagbilao, Municipality of Pagbilao and PAGASA Official Website.</span>
  </div>
  ...
  <script type="module">
    ...
    function fmtTimeISO(s){try{return new Date(s).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})}catch(e){return'—'}}
    ...
    function renderWeather(data){
      ...
      const i = 0;
      const temp = h.temperature_2m ? Math.round(h.temperature_2m[i])+'°C' : '—';
      const humid = h.relativehumidity_2m ? Math.round(h.relativehumidity_2m[i])+'%' : '—';
      const wind = h.wind_speed_10m ? h.wind_speed_10m[i].toFixed(1)+' km/h' : '—';
      const dir = h.wind_direction_10m ? Math.round(h.wind_direction_10m[i])+'°' : '—';
      const clouds = h.cloudcover ? Math.round(h.cloudcover[i])+'%' : '—';
      const rainChance = h.precipitation_probability ? Math.round(h.precipitation_probability[i])+'%' : '—';
      ...
      if(h.time){
        for(let j=1; j<=24; j++){
          ...
          const tTemp = h.temperature_2m ? Math.round(h.temperature_2m[j])+'°C' : '—';
          const tHum = h.relativehumidity_2m ? Math.round(h.relativehumidity_2m[j])+'%' : '—';
          const tWind = h.wind_speed_10m ? h.wind_speed_10m[j].toFixed(1)+' km/h' : '—';
          const tCloud = h.cloudcover ? Math.round(h.cloudcover[j])+'%' : '—';
          const tRain = h.precipitation_probability ? Math.round(h.precipitation_probability[j])+'%' : '—';
    ...
    function renderSunTimes(data){
      ...
      const sunrise = data.daily.sunrise?.[0] ? fmtTimeISO(data.daily.sunrise[0]) : '—';
      const sunset = data.daily.sunset?.[0] ? fmtTimeISO(data.daily.sunset[0]) : '—';
    ...
    async function renderBarangays(){
      ...
      for(const b of barangays){
        try{
          ...
          const temp = c.temperature_2m ? `${Math.round(c.temperature_2m)}°C`:'—';
          const humid = c.relative_humidity_2m ? `${Math.round(c.relative_humidity_2m)}%`:'—';
          const wind = c.wind_speed_10m ? `${c.wind_speed_10m.toFixed(1)} km/h`:'—';
          const cloud = c.cloud_cover ? `${Math.round(c.cloud_cover)}%`:'—';
          const rainChance = c.precipitation_probability!==undefined?`${Math.round(c.precipitation_probability)}%`:'—';
          const sunrise = d.sunrise ? fmtTimeISO(d.sunrise[0]):'—';
          const sunset = d.sunset ? fmtTimeISO(d.sunset[0]):'—';
          ...
          card.innerHTML = `
            ...
            <div class="grid grid-cols-2 gap-2 text-sm">
              <div>🌡️Temp: <strong>${temp}</strong></div>
              <div>🌧️Humidity: <strong>${humid}</strong></div>
              <div>🌬️Wind: <strong>${wind}</strong></div>
              <div>☁️Cloud / Rain: <strong>${cloud} / ${rainChance}</strong></div>
